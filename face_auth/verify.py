...
class FaceVerifier:
    def __init__(self, encodings_path="assets/known_faces/encodings.pkl", video_source=0):
    ...
    def run(self):
        ...
        while True:
            ...
            # Tracking if the authorized user is still present in this frame
            ...
            for (top, right, bottom, left), landmarks, encoding in zip(face_locations, face_landmarks_list, face_encodings):
                ...
                cv2.putText(frame, f"{name.title()} ({1-match_distance:.1%})", (left + 6, bottom - 6), 
            ...
            # 3. Auto-Lock Reset (If authorized user leaves the frame)
